...
const oauthCache = {}
let exchangeCodeExpiryTime = 20000
...
module.exports = {
    init: function(adminAuthSettings, _storage) {
        ...
        exchangeCodeExpiryTime = adminAuthSettings.exchangeCodeExpiryTime || 20000; // 20 seconds in milliseconds
    ...
    generateAuthCode: function(accessToken) {
        const code = crypto.randomBytes(16).toString('hex');
        oauthCache[code] = {
            accessToken,
            expires: Date.now() + exchangeCodeExpiryTime
        }
        return Promise.resolve(code);
    },
    exchangeAuthCode: function(code) {
        const meta = oauthCache[code];
        delete oauthCache[code];
        if (meta && meta.expires > Date.now()) {
            return Promise.resolve(meta.accessToken);
        } else {
