...
function completeGenericStrategyAuth(req,res) {
    ...
    Tokens.generateAuthCode(tokens.accessToken).then((code) => {
        res.redirect(settings.httpAdminRoot + '?code=' + code);
    })
}
...
function exchangeAuthCode(req,res) {
    if (req.body.code) {
        Tokens.exchangeAuthCode(req.body.code).then((accessToken) => {
